...
def test_gemini_provider_token_count():
    """Verifies that the GeminiProvider correctly wraps the underlying generative AI model
    to provide accurate token counts for a given text prompt."""
...
def test_gemini_provider_rate_limiting():
    """Verifies that the provider enforces a minimum time interval between successive API calls
    to respect internal rate limits and avoid throttling from the backend AI service."""
    with patch("google.generativeai.GenerativeModel") as mock_model_class:
        mock_model = mock_model_class.return_value
